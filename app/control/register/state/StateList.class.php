...
class StateList extends TPage
{
    ...
    public function __construct()
    {
        ...
        // add the fields
        $row = $this->form->addFields( [ new TLabel('Pais'),$country_id ],
        ...
        $this->form->addActionLink(_t('New'), new TAction(['StateForm', 'onEdit']), 'fa:plus green');
        ...
        $column_country_id = new TDataGridColumn('country->name', 'Pais', 'left');
        ...
        $action1 = new TDataGridAction(['StateForm', 'onEdit'], ['id'=>'{id}']);
    ...
    public function onInlineEdit($param)
    {
        try
        {
            ...
            $object = new State($key); // instantiates the Active Record
            $object->{$field} = $value;
    ...
    public function onSearch()
    {
        ...
        TSession::setValue(__CLASS__.'_filter_country_id',   NULL);
        ...
        if (isset($data->country_id) AND ($data->country_id)) {
            $filter = new TFilter('country_id', 'like', "%{$data->country_id}%"); // create the filter
            TSession::setValue(__CLASS__.'_filter_country_id',   $filter); // stores the filter in the session
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for State
            ...
            if (TSession::getValue(__CLASS__.'_filter_country_id')) {
                $criteria->add(TSession::getValue(__CLASS__.'_filter_country_id')); // add the session filter
            }
    ...
    public static function Delete($param)
    {
        try
        {
            ...
            TTransaction::open('app'); // open a transaction with database
            $object = new State($key, FALSE); // instantiates the Active Record
